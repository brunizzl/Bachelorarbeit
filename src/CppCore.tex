...
Ein Lambdaparameter ist in der Syntax nicht von anderen Symbolen unterscheidbar\footnote{siehe \ref{subsubsecLambdaSyntax}}, wird intern allerdings mit einer Variante der De Bruijn Indices dargestellt \cite{deBruijn}.
 Jeder neu gebundene Parameter bekommt als Index die Anzahl der weiter außen bzw. in der selben Abstraktion vor ihm gebundenen Parameter. Zur Veranschaulichung wird hier der Index im Tiefsatz mitgeschrieben\footnote{Der Umsetzung sind ausschließlich die Indices bekannt, Namen der Lambdaparameter werden intern nicht gespeichert.}. 
 ...
 Enthält ein Argument $a$ der Funktionsanwendung einer Lambdafunktion $f$ allerdings selbst eine Lambdafunktion $g$, so reicht es für die Auswertung von $f$ mit bisher diskutierten Konzepten im allgemeinen Fall nicht aus, den entsprechenden Lambdaparameter von $f$ einfach durch $a$ zu ersetzen: Sollte diese Ersetzung innerhalb einer Lambdaabstraktion $f'$ stattfinden, müssen alle Lambdaparameter von $g$ im Index um die Anzahl der weiter außen gebundenen Parameter erhöht werden. Als Beispiel ist $a = g = \lambda x_0 .x_0$ die Identität, welche der Funktion $f = \lambda x_0 .\lambda y_1 .x_0(y_1)$ übergeben wird. Teil der Definition von $f$ ist $f' = \lambda y_1 .x_0(y_1)$.
 $$(\lambda x_0 .\lambda y_1 .x_0(y_1))(\lambda x_0 .x_0)$$
 An dieser Stelle muss angemerkt werden, dass durch die De Bruijn Indizierung die explizite Bindung eines Lambdaparameters in der Lambdaabstraktion nicht länger notwendig macht. Dem System ist tatsächlich nur die Anzahl der Parameter einer Abstraktion bekannt. Ehrlicher ist die Darstellung einer Lambdaabstraktionen mit $n$ Parametern als $\lambda [n] .\textit{<Definition>}$. Mit den Parametern geschrieben als Prozentzeichen gefolgt von ihrem De Bruijn Index, ist der problematische Ausdruck geschrieben als
...
\subsubsection{Wert-Mustervariable} \label{subsubsecWertMustervariable}
