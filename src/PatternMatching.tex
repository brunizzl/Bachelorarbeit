...
\ElseIf {$p = (f, p_1, \dots, p_n)$ \textbf{and} $t = (f, t_1, \dots, t_n)$}{
    \For {$k \in \{1, \dots, n\}$}{
...
\ElseIf {$p = (f, \elems p 1 m)$ \textbf{and} $t = (f, \elems t 1 n)$}{
    \If {$u~f$ kommutativ} {
...
\KwIn {$p = (f, \elems p 1 m) \in M$, $t = (f, \elems t 1 n) \in T$,\\
 ...
 \Return {$p$ enthält eine Multi-Matchvariable \textbf{or} alle $t_k$ wurden gematcht}
\end{algorithm}
