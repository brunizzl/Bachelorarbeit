...
Ein Konzept was diese Anforderungen erfüllt ist die Lambdafunktion. Im Kontext eines Terms ist eine Lambdafunktion ein Funktionsymbol, welches nur durch seine Stelligkeit und Abbildungsvorschrift identifiziert ist. Die Abbildungsvorschrift selbst ist ein Term, welcher neben sonst erlaubten Konstantensymbolen noch vom Lambda gebundene Variablen enthalten kann. Diese Variablen sind in der Textdarstellung zwar benannt, intern wird zur Identifizierung jedoch ihr DeBrujin Index genutzt. Für ein Lambda der Stelligkeit $n \in \mathbb N$ ersetzt der Parameter mit Index $k \in \{0, 1, \dots, n - 1\}$ bei der Auswertung der Funktionsanwendung der Lambdafunktion die Vorkommnisse der gebundenen Variable mit DeBrujin Index $k$ in der Abbidungsvorschrift des Lambdas. 


\newtheorem{dLambda}[bsp]{Definition}
\begin{dLambda}
Eine besondere Klasse von Funktionssymbolen und Konstantensymbolen ist die der Lambdas. Die Notation wird eingeleutet durch ein kleines Lambda, gefolgt von der Nennung der gebundenen Variablen. Der Term, der die Abbildungsvorschrift beschreibt folgt als letztes und ist durch ein Punkt von den Variablenbindungen getrennt. 

Als Beispiel bindet das Funktionssymbol $f = \lambda x y.(\texttt{pow}, x, y)$ die Variablen $x$ und $y$. Die Abbildungsvorschrift von $f$ ist $(x, y) \mapsto (\texttt{pow}, x, y)$, alternativ könnte man also schreiben $f = \texttt{pow}$. Wichtig ist, dass die hier definierten Lambdas in Abweichung vom Lambdakalkül auch mehrere Parameter direkt abbilden können. Während die Schreibweise eines einzelnen kleinen Lambdas gefolgt von mehreren Variablennamen in der Literatur also nur eine Kurzschreibweise für ein Lambda, welches den ersten Variablennamen bindet und auf ein Lambda, welches den zweiten Variablennamen bindet ist, werden hier tatsächlich mehrere Variablennamen von nur einem Lambda gebunden. Definiert man also $g = \lambda x.\lambda y.(\texttt{pow}, x, y)$ ist $f \neq g$, denn eine korrekte Funktionsanwendung von $f$ ist $(f, 1, 2)$, während $((g, 1), 2)$ eine korrekte Funktionsanwendung von $g$ ist.
\end{dLambda}

Ein Lambda ist weiter nicht nur als Funktionssymbol, sondern auch als Konstantensymbol erlaubt, kann also selbst Parameter einer Funktionsanwendung sein. Insbesondere kann diese Funktionsanwendung auch in der Abbildungsvorschrift eines anderen Lambdas stehen. Lambda $\lambda_2$ Ergebnis der Auswerung eines weiteren Lambdas $\lambda_1$ sein und von den in $\lambda_1$ gebundenen Variablen abhängen. Unterscheidung der von $\lambda_1$ und $\lambda_2$ gebundenen Variablen ist eigentliche Aufgabe der DeBrujin Indizierung: Bindet $\lambda_1$ $n \in \mathbf N$ Variablen, so haben die DeBrujin Indizes der in $\lambda_2$ gebundenen Variablen eine Verschiebung von $n$. Wird $\lambda_1$ ausgewertet, werden alle Variablen mit DeBrujin Index größer oder gleich $n$ nicht mit einem Parameter ersetzt, sondern im Index um $n$ erniedrigt. Die Schachtelung von Lambdas kann auf diese Weise beliebig fortgesetzt werden.

Mit Lambdas als Konstantensymbolen ist es erlaubt, als Parameter der Funktionsanwendung eines Lambdas $\lambda_a$ ein weiteres Lambda $\lambda_p$ zu übergeben. Es gibt zwei Möglichkeiten die DeBrujin Indizes der von $\lambda_p$ gebundenen Variablen nach Ersetzung anzupassen. Eine Möglichkeit ist, bei der Ersetzung der gebundenen Variablen während der Auswertung eines Lambdas immer die aktuelle Verschiebung mitzuschreiben und bei Ersetzung einer gebundenen Variable durch ein Lambda diese Verschiebung zu allen gebundenen Variablen im resultierenden Lambda dazuzuaddieren. Problem dieses Ansatzes ist zum einen, dass dann nicht nur getestet werden muss ob ein Parameter ein Lambda ist, sondern auch ob ein Parameter ein Lambda enthält. Diese Operation ist in ihrer Komplexität linear in der Anzahl der Funktionsanwendungen und Konstantensymbole des Parameters und deswegen unerwünscht. 
