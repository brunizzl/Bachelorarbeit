...
\ElsIf {$t = (f, t_1, \dots, t_n)$ mit $u_n~f$ assoziativ}
...
\ElsIf {$t = (f, t_1, \dots, t_n)$ und $u_n~f$ assoziativ}
	\If {$f~u_n$ kommutativ}
...
\subsection{Naiver Matchalgorithmus}
...
\State \textbf{let} $v \colon X \rightharpoonup T,~ x \mapsto \bot$
\State \textbf{return} $\mathrm{matchAlgorithm}(m, t, v)$
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1] %[1] -> jede zeile nummeriert
\Require $m \in M$, $t \in T$, $v \colon X \rightharpoonup T$
\If {$m \in X$}
    \If {$v~m = \bot$}        
        \State $(v~m) \leftarrow t$ 
        \State \textbf{return} $(\mathrm{true}, v)$ 
    \Else 
        \State \textbf{return} $(v~m = t, v)$
    \EndIf
\ElsIf {$m \in C \setminus X$}
        \State \textbf{return} $(m = t, v)$    
\ElsIf {$m = f(m_1, \dots, m_n)$ und $t = f(t_1, \dots, t_n)$}
    \ForAll {$k \in \{1, \dots, n\}$}
        \State $(success, v) \leftarrow \mathrm{simpleMatchAlgorithm}(m_k, t_k, v)$
        \If {$\mathrm{not}~success$}
            \State \textbf{return} $(\mathrm{false}, v)$
        \EndIf
    \EndFor
    \State \textbf{return} $(\mathrm{true}, v)$  
\EndIf
...
Wenn das Match streng definiert ist, also als Unterschied zwischen einem Muster $m$ und einem Literal $l$ aussschließlich darin bestehen darf, dass Teilterme von $l$ in $m$ durch eine Mustervariable repräsentiert werden, ist ein einfacher Matchalgorithmus fast trivial. Auf der in Algorithmus \ref{simpleMatchAlgorithm} genutzten Grundidee Muster und Literal parallel rekursiv abzulaufen basieren aber alle späteren Algorithmen dieses Kapitels.
