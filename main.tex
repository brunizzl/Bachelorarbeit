...
\KwData{$m \in M$, $t \in T$}
\Begin{
    \textbf{let} $v \colon X \rightharpoonup T,~ x \mapsto \bot$\;
    \Return {$\mathrm{matchAlgorithm}(m, t, v)$}
}
\end{algorithm}
...
\KwData {$m \in M$, $t \in T$, $v \colon X \rightharpoonup T$}
\Begin{
    \If {$m \in X$ \textbf{and} $v~m = \bot$}{
        $(v~m) \leftarrow t$\;
        \Return {$(\mathrm{true}, v)$}
    }
    \ElseIf {$m \in X$ \textbf{and} $v~m \neq \bot$}{
        \Return {$(v~m = t, v)$}
    }
    ...
    \ElseIf {$m = f(m_1, \dots, m_n)$ \textbf{and} $t = f(t_1, \dots, t_n)$}{
        \For {$k \in \{1, \dots, n\}$}{
            $(s_k, v) \leftarrow \mathrm{simpleMatchAlgorithm}(m_k, t_k, v)$\;
            \If {$\mathrm{not}~s_k$}{
                \Return {$(\mathrm{false}, v)$}
            }
        }
        \Return {$(\mathrm{true}, v)$}  
    }
