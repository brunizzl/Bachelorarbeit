...
\ElsIf {$t = (f, t_1, \dots, t_n)$ mit $f~u_n$ assoziativ}
...
\ElsIf {$t = (f, t_1, \dots, t_n)$ und $f~u_n$ assoziativ}
	\If {$f~u_n$ kommutativ}
		\While {$t = (f, xs..., a, ys..., b, zs...) $ und $ u~f~(a, b) = c \in C$}
			\State $t \leftarrow (f, xs..., c, ys..., zs...)$
		\EndWhile
	\Else
		\While {$t = (f, xs..., a, b, ys...) $ und $ u~f~(a, b) = c \in C$}
...
$$\mathrm{eval}(u, v)~(f, a, b) = c \in C \implies (f, xs..., a, b, ys...) \mapsto (f, xs..., c, ys...)$$
Ist $f$ zudem kommutativ gilt 
$$\mathrm{eval}(u, v)~(f, a, b) = c \in C \implies (f, xs..., a, ys..., b, zs...) \mapsto (f, xs..., c, ys..., zs...)$$
...
\If {$t = (f, t_1, \dots, t_n)$}
	\For {$i \in \{1, \dots, n\}$}
		\State $t_i \leftarrow \mathrm{normalize}~t_i$
	\EndFor
\EndIf
\State $t \leftarrow \mathrm{flatten}~t$
\State $t \leftarrow \mathrm{combine}~t$
\If {$t = (f, t_1, \dots, t_n)$ mit $f~u$ kommutativ}
...
\section{Weitere Konzepte mit Relevanz für die Umsetzung} \label{secHilfUmsetzungInCpp}
...
Ein Konzept was diese Anforderungen erfüllt ist die Lambdafunktion. Im Kontext eines Terms ist eine Lambdafunktion ein Funktionsymbol, welches nur durch seine Stelligkeit und Abbildungsvorschrift identifiziert ist. Die Abbildungsvorschrift selbst ist ein Term, welcher neben sonst erlaubten Konstantensymbolen noch vom Lambda gebundene Variablen enthalten kann. 
